...
La alta riqueza de cómputo de la morfología del sistema NMS, sobre la cual por su composición neuronal, es posible estructurar un controlador jerárquico dotado de cualidades cognitivas, como por ejemplo, una clara diferencia de memorias de corto y largo plazo para definir diferentes tipos de acciones, como reflexivas, de estrategia y predictivas. Hace que éstas propiedades del sistema NMS combinadas con el estado-del-arte de los controladores de humanoides, permitan proponer un modelo sobre el cual se pueda extraer características de forma automática y éstas características, las cuales puedan a su vez, codificar y representar los comportamientos en un lenguaje natural, mediante optimización y satisfacción de criterios, los cuales se encarnan sobre el CNS acoplado al sistema NMS, representando conceptos como el consumo de energía, la intensidad de esfuerzos, el balance, la postura y otros. 

Mas aún estas características sirven para estudiar la propiedad auto-estabilizante del sistema NMS. Para tareas donde los sistemas son auto-estabilizantes, es decir que no requieren de un control activo para llegar o regresar a una posición de seguridad, generalmente la transferencia de políticas simuladas trabaja bien en el mundo real. Cuando el sistema físico real realiza una tarea auto-estabilizante, los algoritmos de RRL se puede utilizar directamente y madurar el controlador mediante \emph{ensayos mentales}\footnote{mental rehearsal}. En tareas inestables una pequeña variación puede tener consecuencias drásticas y la transferencia de políticas resulta en malos desempeños, por lo tanto es importante definir algunas reglas acerca de la auto-estabilización de un humanoide con sistema NMS, ya sea virtual o no, para poder utilizar los beneficios de los ensayos mentales.

Un ejemplo de característica usado en robótica de humanoides, es el ZMP, que duró años la ingeniería en proponer para lograr una locomoción sub-óptima. Este es un claro ejemplo de una característica diseñada manualmente que vigila el balance de la caminata y con base en su valor actual y el valor deseado, tomar una decisión de control. El análisis y extracción de características es una herramienta clave para problemas abiertos como: (1) la búsqueda de restricciones virtuales de forma \emph{on-line} en HZD; o (2) caracterizar mejor la fase de doble soporte; como se propone en~\cite{Grizzle2014}. Estos problemas son tratables al analizar las señales del sistema NMS como representación del lazo sensoriomotor (SML), según se propondrá más adelante en la metodología.

% para justificación el sistema NMS permite trabajar diferentes escalas de retardos
El sistema NMS permite configurar diferentes retardos de actuación y sensado con un significado neuro-mecánico. Debido a estos retardos, las acciones puede que no tengan efectos instantáneos y que solo sean observables varios intervalos de tiempo posteriores, estos retardos violan los supuestos de MDPs. Lo cual hace este modelo interesante para analizar transferencias de políticas del RRL utilizando ensayos mentales. Las estructuras de diferentes manejos del tiempo, permiten también estructurar el controlador en aciones reflexivas y otras que requieren más estrategia~\cite{Maniadakis2014}. 

\section{Algunas aplicaciones, motivaciones y problemas abiertos}
\label{sec:motivaciones}
Una aplicación reciente que muestran la importancia de investigar sobre la problemática planteada, es sobre prótesis transfemorales, que estudia la recuperación del balance de la marcha utilizando un control NM~\cite{Thatte2015}, logrando una mayor robustez en la marcha comparado con los controladores de impedancia, los cuales se destacan en prótesis actuadas. Esta comparación se realiza mediante simulación y hardware, obteniendo como resultado una mejor comunicación entre el usuario y la prótesis, pero al final se plantea que es necesario estudiar mejor dicha política de control analizando la marcha anormal de un amputado. Un trabajo similar es comprobado por \cite{Dzeladini2016}, para un exoesqueleto que mejora la eficiencia del talon durante la marcha empleando un control neuromuscular.
...
Utilizar modelos aproximados, como ZMP, IPM o HZD, sirve para: verificar y probar algoritmos en simulación, establecer proximidad a soluciones teóricamente óptimas, calcular gradientes aproximados para el mejoramiento de políticas locales, identificar estrategias para recolectar más datos y realizar ensayos mentales. 

% para justificación del Deep learning, RL y el IRL 
En lugar de proponer funciones de recompensa simples binarias, frecuentemente se incluye recompensas intermedias, para guiar un proceso de aprendizaje a una solución razonable, este diseño de recompensa se conoce como \emph{reward shaping}. Existe un intercambio entre la complejidad de la función de recompensa y la complejidad del problema de aprendizaje. El comportamiento humano que parece respetar las prioridades de tiempo y riesgo, puede lograrse mediante una señal de recompensa que describa el mapeo de las características que describen cada estado. Cuando las funciones de recompensa son lineales en el conjunto de características, esas funciones imponen una limitación restrictiva sobre el comportamiento en la solución del problema IRL. Las relaciones no-lineales de las características para la formación de las funciones de costo y el diseño automático de características combinado con el conocimiento ya desarrollado en la locomoción humanoide, son una fuerte motivación para entender nuevas características que ayuden a la solución del problema de la locomoción bípeda~\cite{Finn2016}. La noción de construir costos o políticas complejas, a partir de unas más simples, las cuales son fácilmente resueltas por el control óptimo, al explorar funciones de recompensa parametrizadas, es un reto actual en el RRL y el SOC. Por lo general, las políticas complejas derivadas de la adaptación de una función de recompensas, para problemas simples y de humanoides 3D articulares de control óptimo, han sido utilizadas en conjunto con PS (denominado también como \emph{actor-only}) y muchas veces sobre en una estructura de \emph{actor-critico}~\cite{Duan2016}, sin embargo un humanoide con sistema NMS con asimetrías y limitaciones articulares es un problema abierto a estudio. 

% para justificación de estructuras de LSTM en secuencias de posturas
La locomoción como un modelo de secuencias de posturas y su relación con estructuras del Deep Learnning usando (1) las \emph{memorias de corto y largo plazo} (LSTM) para aspectos temporales; Deep RBM y \emph{Deep Belief Networks} (DBN) para aproximación de distribuciones complejas. Según la revisión del estado del arte, es un tema que esta sin aplicar aún a los humanoides.%Importante: Resaltar el aporte del objetivo general en la ciencia.
(3) De igual manera el analizar el lazo sensoriomotor, que modela el sistema NMS, mediante el Control Óptimo Estocástico y el RRL basado en el modelos gráficos, para la extracción de características; Los tres temas anteriores, se cree que aportaría en biomecánica y neuromecánica, como modelo general para análisis y diseño de por ejemplo prótesis o el análisis de marcha, como se explora en los trabajos de \cite{Thatte2015,Dzeladini2016}.
